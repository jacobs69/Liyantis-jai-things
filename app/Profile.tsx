...
export default function ProfileScreen() {
  ...
  const soldProjects = projectsData.projects.filter(p => p.status === "Sold").length;
  
  const [selectedCurrency, setSelectedCurrency] = useState(userData.preferences.currency);
...
const ProfileSummary = ({ userName, totalProjects, soldProjects }: { userName: string; totalProjects: number; soldProjects: number }) => {
  return (
    <View style={styles.summaryContainer}>
      <View style={styles.avatarWrapper}>
        <View style={styles.avatarContainer}>
          <Image 
            source={{ uri: `https://api.dicebear.com/7.x/avataaars/png?seed=${userName}&backgroundColor=b6e3f4` }} 
      ...
      <View style={styles.statsContainer}>
        <Text style={styles.statText}>Projects: <Text style={styles.statHighlight}>{totalProjects}</Text></Text>
        <Text style={styles.statText}>Sold: <Text style={styles.statHighlight}>{soldProjects}</Text></Text>
      </View>
    </View>
  );
};

const PersonalInfoSection = ({ email, phone }: { email: string; phone: string }) => {
  return (
